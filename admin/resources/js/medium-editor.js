    ...
    */
    ...
    var PasteHandler = MediumEditor.Extension.extend({
        ...
        handleKeydown: function (event, editable) {
            if ( $(editable).find('input').length == 0 || !$(editable).find('input').is(':focus') ){
...
(function () {
    ...
    function handleBlockDeleteKeydowns(event) {
        ...
        /**
         * This if is customized
         *
         */
        if ( node.classList.contains('space-before') ){
            if ( MediumEditor.util.isKey(event, MediumEditor.util.keyCode.BACKSPACE) ){
                // Do nothing
                event.preventDefault();
            } else if ( MediumEditor.util.isKey(event, MediumEditor.util.keyCode.DELETE ) ){
                // Delete class space-before
                node.classList.remove('space-before');
            } else if ( MediumEditor.util.isKey(event, MediumEditor.util.keyCode.ENTER ) ){
                node.classList.remove('space-before');
                event.preventDefault();
            }
        // } else if ( node.previousElementSibling && node.previousElementSibling.previousElementSibling ){
        //     if ( node.previousElementSibling.classList.contains('img-container') &&
        //         node.previousElementSibling.previousElementSibling.classList.contains('img-container') ){
    
        //           p = this.options.ownerDocument.createElement('p');
        //           p.innerHTML = '<br>';
        //           node.previousElementSibling.parentNode.insertBefore(p, node.previousElementSibling);
        //     }
